...
| 2026-09-17 | **AH basket writes found via GraphQL.** The website's quantity stepper calls `mutation basketItemsUpdate` on `www.ah.nl/gql`; the same schema is served on the mobile host `api.ah.nl/graphql`, which accepts the app bearer token and is reachable from the box (`basket` query verified read-only). The connector got `graphql()`, `basket()`, `basketItemsUpdate()`; the store capability now has `basket_add`, `basket_plan` and `basket_fill_from_list` behind the confirm gate (`AH_BASKET_WRITE=true` default). First live write to be triggered by the family from Telegram. Details in [STORE-AH.md](./STORE-AH.md). |
